  ...
  analyze() {
    for (let event of this.game.events) {
      if (event.type == "START_OF_TURN") {
        ...
        if (
          event.action == "steal" &&
          this.isHumanPlayer(this.startEvent.whoseTurn)
        ) {
          this.recordStat(
            "steal",
            event.target,
            this.playerHasInfluence(this.startEvent.whoseTurn, "captain")
          );
        }
      } else if (event.type == "BLOCK") {
        if (
          event.blockingRole == "captain" &&
          this.isHumanPlayer(event.blockingPlayer)
        ) {
          this.recordStat(
            "block_steal",
            this.startEvent.whoseTurn,
            this.playerHasInfluence(event.blockingPlayer, "captain")
          );
        }
      }
    }
...
const stream = fs
  .createReadStream("games_full.json.gz")
...
function printStats() {
  for (let key of Object.keys(stats).sort()) {
    ...
    console.log(
      `${key}:\t${stats[key][0]} (${truthPercent}%) truths, ${stats[key][1]} (${bluffPercent}%) bluffs`
    );
  }
}
