  ...
  ready = treasonDb
    ...
    .then(function () {
      if (recreateViews || updateViews) {
        debug(
          "Recreating views because " +
            (updateViews ? "view version was updated" : "of --recreate-views")
        );
        return treasonDb
          .save("_design/games", {
            by_winner: {
...
function recordPlayerDisconnect(playerId) {
  return ready.then(function () {
    return treasonDb.get(playerId).then(function (player) {
      if (player) {
...
function updateResults(gameData) {
  ...
  for (let player of gameData.playerRank) {
    ...
    stats[player].games++;
    stats[player].percent = Math.floor(
      (100 * stats[player].wins) / stats[player].games
    );
...
function calculateAllStats() {
  ...
  return Promise.all([
    ...
    for (let win of wins) {
      ...
      newStats[playerId].percent = Math.floor(
        (100 * winStats.wins) / newStats[playerId].games
      );
    }
...
function timeApi(fn) {
  return function () {
    ...
    function done(err) {
      ...
      debug(
        fn.name + " took " + ms(end - start) + " " + ((err && err.stack) || "")
      );
    }
    debug(fn.name + "...");
