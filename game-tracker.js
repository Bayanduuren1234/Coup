...
GameTracker.prototype.challenge = function (challenger, challenged, success) {
  this.events.push({
    type: success
      ? GameTracker.TYPE_CHALLENGE_SUCCESS
      : GameTracker.TYPE_CHALLENGE_FAIL,
    ...
    switch (event.type) {
      ...
      case GameTracker.TYPE_GAME_OVER:
        ...
        event.playerStates.forEach(function (playerState) {
          ...
          bytes[offset++] =
            (tracker.encodeInfluence(playerState.influence[0]) << 4) |
  ...
  while (offset < buffer.length) {
    ...
    switch (type) {
      ...
      case GameTracker.TYPE_START_OF_TURN:
        ...
        debug(
          `>   found ${statesFound} states, ${gameInfo.playerCount} players`
        );
        ...
        while (offset < buffer.length) {
          ...
          try {
            ...
            if (type2 == GameTracker.TYPE_PLAYER_LEFT) {
              ...
              if (
                this.looksLikePlayerStateEvent(
                  buffer,
                  offset,
                  lastStart,
                  gameInfo.playerCount
      ...
      case GameTracker.TYPE_BLOCK:
        ...
        if (lastAction && typeof lastAction.target == "number") {
          ...
          if (!lastAction) {
            ...
            throw new Error(
              `Unexpected blocked action: ${lastAction && lastAction.action}`
            );
          }
        }
        lastBlock = event;
...
GameTracker.prototype.readPlayerStates = function (
  buffer,
  offset,
  event,
  playerCount
...
GameTracker.prototype.looksLikePlayerStateEvent = function (
  buffer,
  offset,
  lastStart,
  playerCount
  ...
  if (
    type != GameTracker.TYPE_START_OF_TURN &&
    type != GameTracker.TYPE_GAME_OVER
  ) {
    return false;
  }
  const event = {};
...
GameTracker.prototype._debugEventByte = function (offset, byte, showType) {
  ...
  debug(
    offset,
    showType ? GameTracker.EventTypes[hw[1]] : "",
    byte,
    hw.reverse(),
    qw.reverse()
  );
...
GameTracker.prototype.validateActionEvent = function (event, playerStateCount) {
  ...
  if (ACTION_INFO[event.action].targeted && event.target >= playerStateCount) {
    throw new Error(
      `Unknown player ${event.target} targeted by action ${event.action}`
    );
  }
};
...
GameTracker.prototype.decodeRole = function (roleCode) {
  if (roleCode === 1) {
    ...
    return this.gameInfo.roles.indexOf("ambassador") >= 0
      ? "ambassador"
      : "inquisitor";
  } else if (roleCode === 5) {
    return "contessa";
...
GameTracker.prototype.identifyPlayers = function (events, game) {
  ...
  if (playerDeaths.length !== game.players - 1) {
    console.log(
      `wrong number of deaths: ${playerDeaths.length}, for players: ${game.players}`
    );
  ...
  if (game.players !== playerDeaths.length) {
    console.log(
      `wrong number of winners: ${playerDeaths.length}, for players: ${game.players}`
    );
    return { playerIds: null, winners: null };
  }
