...
vm = {
  ...
  globalChatMessages: ko.observableArray(["Тавтай морилно уу"]), // The global chat messages that have been received.
...
socket.on("connect", function () {
  ...
  socket.on("globalchatmessage", function (data) {
    ...
    globalMessageContainer[0].scrollTop =
      globalMessageContainer[0].scrollHeight;
  });
...
function isInvalidPlayerName() {
  if (
    ...
    // !vm.playerName().match(/^[а-яА-Я0-9_ !@#$*]+$/) ||
    !vm.playerName().trim()
  ) {
    alert("Хүчинтэй нэр оруулна уу");
    return true;
  }
  if (vm.playerName().length > 30) {
    alert("Нэр хэтэрхий урт байна");
    return true;
  }
  return false;
}
