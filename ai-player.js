...
var aiPlayerNames = fs
  .readFileSync(__dirname + "/names.txt", "utf8")
  .split(/\r?\n/);
...
function createAiPlayer(game, options) {
  ...
  function onStateChange(s) {
    ...
    if (state.state.name === stateNames.WAITING_FOR_PLAYERS) {
      ...
      var delay = rand.intBetween(
        options.moveDelay - options.moveDelaySpread,
        options.moveDelay + options.moveDelaySpread
      );
  ...
  function onStateChangeAsync() {
    ...
    if (
      ...
      state.state.playerToReveal == state.playerIdx
    ) {
      revealByProbability();
    } else if (
      state.state.name == stateNames.EXCHANGE &&
      currentPlayer == aiPlayer
    ) {
      exchange();
    }
  ...
  function onHistoryEvent(message) {
    ...
    if (
      message.indexOf(" challenged") > 0 &&
      lastRoleClaim &&
      claims[lastRoleClaim.playerIdx]
    ) {
    ...
    if (
      message.indexOf(" successfully challenged") > 0 &&
      lastRoleClaim &&
      calledBluffs[lastRoleClaim.playerIdx]
    ) {
  ...
  function shouldChallenge() {
    ...
    if (
      state.state.name == stateNames.ACTION_RESPONSE &&
      calledBluffs[state.state.playerIdx] &&
      calledBluffs[state.state.playerIdx][claimedRole]
    ) {
    ...
    if (
      state.state.name == stateNames.BLOCK_RESPONSE &&
      calledBluffs[state.state.target] &&
      calledBluffs[state.state.target][claimedRole]
    ) {
    ...
    if (
      ...
      if (
        ...
        calledBluffs[state.playerIdx]["contessa"]
      ) {
        return true;
      }
  ...
  function actionIsWorthChallenging() {
    ...
    if (
      (state.state.action == "steal" || state.state.action == "assassinate") &&
      (state.state.playerIdx == state.playerIdx ||
        state.state.target == state.playerIdx)
    ) {
      return true;
    }
    return false;
  ...
  function countRevealedRoles(role) {
    ...
    for (var i = 0; i < state.numPlayers; i++) {
      for (var j = 0; j < state.players[i].influence.length; j++) {
        if (
          state.players[i].influence[j].revealed &&
          state.players[i].influence[j].role === role
        ) {
          count++;
        }
      }
  ...
  function getBlockingRole() {
    ...
    if (
      state.state.action == "foreign-aid" ||
      state.state.target == state.playerIdx
    ) {
  ...
  function playOurTurn() {
    ...
    if (aiPlayer.cash >= 7) {
      ...
      influence.indexOf("captain") >= 0 &&
      captainTarget() != null &&
      !randomizeChoice()
    ) {
      playAction("steal", captainTarget());
    } else if (
      influence.indexOf("duke") >= 0 &&
  ...
  function shouldBluff(actionNameOrRole) {
    ...
    if (
      Object.keys(claims[state.playerIdx]).length > 2 &&
      !claims[state.playerIdx][role]
    ) {
    ...
    if (influence.length > 1) {
      ...
      chosenInfluence =
        influenceProbability[
          rand.intBetween(0, influenceProbability.length - 1)
        ];
    }
    command({
      command: "reveal",
  ...
  function canBlock(playerIdx, actionName) {
    return (
      lodash.intersection(
        actions[actionName].blockedBy,
        getClaimedRoles(playerIdx)
      ).length > 0
    );
  }
  ...
  function playersByStrength() {
    ...
    return indices.sort(function (a, b) {
      ...
      if (infa != infb) {
        ...
        return md5(randomNumber + state.players[a].name) <
          md5(randomNumber + state.players[b].name)
          ? -1
          : 1;
      }
    });
  ...
  function simulate(bluffedRole) {
    ...
    function otherCanBlock(actionName) {
      return (
        lodash.intersection(roles[other], actions[actionName].blockedBy)
          .length > 0
      );
    }
    function canSteal() {
    ...
    function canAssassinate() {
      return (
        roles[turn].indexOf("assassin") >= 0 && !otherCanBlock("assassinate")
      );
    }
