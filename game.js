...
module.exports = function createGame(options) {
  ...
  var reaperHandle = setTimeout(function () {
    debug("Тоглогч олдсонгүй. Өрөөг хааж байна");
    destroyGame();
  }, 120000);
  ...
  function playerLeft(playerIdx) {
      ...
      if (!playerState.isObserver) {
        ...
        for (var j = 0; j < influence.length; j++) {
          if (!influence[j].revealed) {
            historySuffix.push(
              format("{%d} устгагдсан %s", playerIdx, influence[j].role)
            );
    ...
    addHistory(
      "тоглогч-гарсан",
      nextAdhocHistGroup(),
      playerState.name + " тоглолтоос гарлаа"
    );
    for (var k = 0; k < historySuffix.length; k++) {
      addHistory("тоглогч-гарсан", curAdhocHistGroup(), historySuffix[k]);
    }
    if (onlyAiLeft()) {
      destroyGame();
    }
  ...
  function playerReady(playerIndex) {
    ...
    if (!playerState.isReady) {
      ...
      addHistory(
        "тоглоход-бэлэн",
        nextAdhocHistGroup(),
        playerState.name + " дахин тоглоход бэлэн байна"
      );
    }
  }
  ...
  function addAiPlayer() {
    if (countReadyPlayers() >= state.maxPlayers) {
      throw new GameException(
        "Хиймэл оюун нэмэх боломжгүй: Тоглогч дүүрсэн байна."
      );
    }
  ...
  function afterPlayerDeath(playerIdx) {
    ...
    addHistory(
      "тоглогч-хасагдсан",
      nextAdhocHistGroup(),
      "{%d} тоглолтоос гутамшигтайгаар хасагдсан",
      playerIdx
    );
    checkFreeForAll();
    checkForGameEnd();
  ...
  function challenge(playerIdx, challengedPlayerIdx, challengedRole) {
    ...
    if (proof != null) {
      ...
      var message = format(
        "{%d} {%d}-г шалгахад үнэн байсан тул; {%d} %s-г %s сольсон",
        playerIdx,
        challengedPlayerIdx,
        challengedPlayerIdx,
        oldRoles,
        proof.length == 1 ? "шинэ дүрээр" : "шинэ дүрүүдээр"
      );
