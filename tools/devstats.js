  ...
  views: {
    ...
    odd_games: {
      map: function (doc) {
        if (
          doc.type === "game" &&
          /*doc.humanPlayers === 0 ||*/ doc.humanPlayers > 6
        ) {
          emit(doc.id, doc.id);
        }
      },
...
if (argv.create) {
  treasonDb.save("_design/dev", views).catch(handleErrors);
} else {
  calculateStats()
    .then(function (stats) {
...
function calculateStats() {
  return Promise.all([
    ...
    wins.forEach(function (playerId, data) {
      ...
      stats[playerId].percentHuman =
        stats[playerId].winsHuman / stats[playerId].gamesHuman;
      stats[playerId].percentAI =
        stats[playerId].winsAI / stats[playerId].gamesAI;
    });
